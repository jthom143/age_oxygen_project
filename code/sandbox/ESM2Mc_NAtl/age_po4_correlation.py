...
po4 = iris.load_cube(PATH+'po4.nc')
po4 = po4[-500:]
...
#po4_linew = interpolate_to_linew(po4, 'Phosphate')
#age_linew = interpolate_to_linew(age, 'age')
#neutral_rho_linew = interpolate_to_linew(neutral_rho, 'neutral_rho')

#iris.save(po4_linew, 'po4_linew.nc')
#iris.save(age_linew, 'age_linew.nc')
#iris.save(neutral_rho, 'neutral_rho_linew.nc')


### Load Line W Data
po4_linew = iris.load_cube('po4_linew.nc')
...
po4_clim = po4_linew.collapsed('time', iris.analysis.MEAN)


# Calculate Correlation between age and oxygen
test = iris.analysis.stats.pearsonr(po4_linew, age_linew, corr_coords=['time'])
...
plt.contourf(po4_linew.coord('distance').points, po4_linew.coord('tcell pstar').points, test.data,clevs, cmap = 'RdBu_r')
cb = plt.colorbar()
cb.set_label('Pearson Correlation Coefficient', fontsize = 14)
CS = plt.contour(po4_linew.coord('distance').points, po4_linew.coord('tcell pstar').points, neutral_rho_linew_mean.data-1000,
                 levels = [26.0, 26.5, 27.0, 27.5, 28.0], colors = 'k')
plt.clabel(CS, fontsize=9, inline=1)
plt.ylim([0,2000])
ax.invert_yaxis()
plt.title('(a) Age-PO4 Correlation', fontsize = 14)
plt.ylabel('Depth [m]', fontsize = 14)

plt.subplot(2,2,3)
po4_clim = po4_linew.collapsed('time', iris.analysis.MEAN)
clevs = np.arange(0, 1.65, 0.05)
ax = plt.gca()
plt.contourf(age_linew.coord('distance').points, po4_linew.coord('tcell pstar').points, po4_clim.data*1e6, clevs,
             cmap = cmaps.viridis, extend = 'both')
cb = plt.colorbar()
CS = plt.contour(age_linew.coord('distance').points, po4_linew.coord('tcell pstar').points, neutral_rho_linew_mean.data - 1000, colors = 'k' ,
...
plt.ylabel('Depth [m]', fontsize = 14)
plt.title('(b) Phosphate Climatology', fontsize = 14)
...
plt.contourf(po4_linew.coord('distance').points, po4_linew.coord('tcell pstar').points, age_clim.data, clevs, cmap = cmaps.viridis, extend = 'both')
cb = plt.colorbar()
CS = plt.contour(age_linew.coord('distance').points, po4_linew.coord('tcell pstar').points, neutral_rho_linew_mean.data - 1000, colors = 'k' ,
                 levels = [26.0, 26.5, 27.0, 27.5])
plt.ylim([0,2000])
...
plt.xlabel('Distance [km]', fontsize = 14)
plt.savefig('po4_correlation_clim_on_linew.pdf')
...
po4_linew_clim = po4_linew.collapsed('time', iris.analysis.MEAN)
...
plt.figure()
plt.scatter(age_linew_clim[:,:].data.flatten(), po4_linew_clim[:,:].data.flatten()*1e6, color = 'b')
plt.show()
